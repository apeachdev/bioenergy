...
if ( ! function_exists( 'cargopress_theme_setup' ) ) {
	function cargopress_theme_setup() {
		...
		add_image_size( 'cargopress-jumbotron-slider-s', 425, 200, true );
...
if ( ! function_exists( 'cargopress_enqueue_styles' ) ) {
	function cargopress_enqueue_styles() {
		...
		if ( CargoPressHelpers::is_woocommerce_active() ) {
			wp_enqueue_style( 'cargopress-woocommerce', get_stylesheet_directory_uri() . '/woocommerce.css' , array() , CARGOPRESS_WP_VERSION );
		}
...
if ( ! function_exists( 'cargopress_enqueue_scripts' ) ) {
	function cargopress_enqueue_scripts() {
		...
		if ( true === CARGOPRESS_DEVELOPMENT ) {
			wp_enqueue_script( 'cargopress-main', get_template_directory_uri() . '/assets/js/main.js', array(
				'jquery',
				'underscore',
				'requirejs',
				'cargopress-gmaps',
			), CARGOPRESS_WP_VERSION, true );
		}
		else {
			wp_enqueue_script( 'cargopress-main', get_template_directory_uri() . '/assets/js/main.min.js', array(
				'jquery',
				'underscore',
				'cargopress-gmaps',
			), CARGOPRESS_WP_VERSION, true );
